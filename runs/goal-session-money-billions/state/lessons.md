...
## iter-3 — 2026-05-18T08:03:08Z

**Verdict:** GOAL_ACHIEVED
**Lesson:** Closing the J-04 OOS-aware soft gap required the evaluator to OPEN the pixels, not just diff hashes: sha256 byte-distinctness from the J-03 capture is necessary but NOT sufficient (a byte-distinct screenshot can still be the wrong surface). The gap only genuinely closed because `UT-10-j04-oos-insights-zoom.png` was a *content-legible* zoom of the insights pane whose prose verbatim cited "healthy 1.256 WFE" and "OOS results remain negative at -7.22% with a -1.02 Sharpe". Separately: the eager-load anti-goal was low-risk to fix as a one-line `read_iteration_full`→`read_iteration_meta` swap *because* `session_store.py` already physically splits heavy keys into a separate file (`_BULK_KEYS`), so `read_iteration_meta` makes the lightweight contract structurally guaranteed rather than a hand-maintained field allowlist — when resolving a "must not include heavy payload" anti-goal, prefer a reader backed by an on-disk split over a manual filter.
**Applies to:** any future iter whose evidence hinges on screenshot distinctness (require a zoomed, content-legible capture of the *specific claimed surface*, not just a sha256 difference); any iter resolving a "response must exclude payload X" anti-goal (prefer a structurally-guaranteed reader over a hand-filtered allowlist).
