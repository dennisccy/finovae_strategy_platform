...
| **(planned)** Automated-session run state + stop reason — `running` / `queued` / `stopped` / `criteria-met` / `budget-exhausted` | Automated-session controller (Layer 1 — to be built) | Session record via `GET /api/sessions/{id}` (no parallel store) | One canonical status per session; UI status strip and API read the same value; persisted to durable store (survives reload/restart) |
| **(planned)** Budget counters — token spend, USD cost, wall-clock | Immutable cost tracker (Layer 1/2 — to be built) | Session record via `GET /api/sessions/{id}` | One authoritative counter per automated session; hard cap; UI and API read the same tally — never separate counts |
| **(planned)** Robust objective score + best marker | Robust scorer (Layer 2 — to be built), derived from `WalkForwardResult` + `BacktestResult` | Session record via `GET /api/sessions/{id}` | WFE-gated, drawdown-penalized, min-trades floor; selects the single "best" iteration — one score definition, read identically by leaderboard and best badge |

<!-- The three (planned) rows reserve canonical homes for the not-yet-built automated session so Layer 1/2
iterations conform to "single source, write the same artifacts the UI renders." They are forward-looking;
the existing four rows reflect verified, shipped code. -->
