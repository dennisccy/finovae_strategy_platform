...
| **(Layer-1, iter-1)** Automated-session run state + stop reason — `queued` / `running` / `stopped` / `criteria-met` / `budget-exhausted` / `interrupted` | `apps/backend/backend/auto_session.py` (`AutoSessionController`) | `GET /api/sessions/{id}` — new `autoRun` block on the session record (no parallel store; started via `POST /api/auto-sessions`) | One canonical status per session, persisted to `session.json`'s `autoRun` block (survives worker restart + browser reload); orphaned `running` reconciled to `interrupted` on startup. UI status strip and API read the same value |
| **(Layer-1, iter-1)** Budget counters — iterations done, wall-clock seconds; token spend + USD cost | Immutable budget tracker in `apps/backend/backend/auto_session.py` | `GET /api/sessions/{id}` — `autoRun.budget` block | One authoritative counter per automated session. **Iterations + wall-clock are hard-enforced in iter-1** (the loop never starts a round past the cap). **Token/USD are populated best-effort; their hard cap enforcement is Layer-2 (J-13).** UI and API read the same tally — never separate counts |
| **(Layer-1, iter-1)** Robust objective score + best marker (`autoRun.bestIterationId`) | Robust scorer in `apps/backend/backend/auto_session.py`, derived from the canonical `WalkForwardResult` (`POST /api/execute-walk-forward`) + `BacktestResult` | `GET /api/sessions/{id}` — `autoRun.bestIterationId` on the session record | WFE-gated, drawdown-penalized, min-trades floor; selects the single "best" iteration — one definition, read identically by the best badge and (Layer-2) the leaderboard. **Legacy duplicate:** the in-browser `scoreIteration` in `apps/frontend/src/hooks/useBacktest.ts` is pre-existing and is retired when **J-10 (iter-2)** rewires Auto Run to this backend loop — until then it drives only the manual in-browser Auto Run |

<!-- Layer-1 iter-1 lands the first three automated-session rows above (run state + stop reason, budget
counters, robust best-marker) in apps/backend/backend/auto_session.py, written to the SAME file store the UI
renders (no parallel store, no schema fork). Hard token/USD budget enforcement (J-13) and the open-universe
optimizer / leaderboard (J-12, J-14–J-16) remain forward-looking. The other four rows reflect verified,
shipped code. -->
