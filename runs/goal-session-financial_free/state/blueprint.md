...
| **(Layer-1, iter-1)** Robust objective score + best marker (`autoRun.bestIterationId`) | Robust scorer in `apps/backend/backend/auto_session.py`, derived from the canonical `WalkForwardResult` (`POST /api/execute-walk-forward`) + `BacktestResult` | `GET /api/sessions/{id}` — `autoRun.bestIterationId` on the session record | WFE-gated, drawdown-penalized, min-trades floor; selects the single "best" iteration — one definition, read identically by the best badge and (Layer-2) the leaderboard. **Legacy duplicate retired (iter-2):** the in-browser `scoreIteration` and the second in-browser iterate loop in `apps/frontend/src/hooks/useBacktest.ts` have been **removed** — **J-10 (iter-2)** rewired Auto Run to start this backend loop (`POST /api/auto-sessions`), so the backend `RobustScorer` is now the **sole** "best" definition and the only iterate engine; the UI only reads `autoRun.bestIterationId`. **(iter-7, J-16) Per-candidate leaderboard:** the SAME `RobustScorer`'s per-candidate output is now also surfaced as `autoRun.leaderboard` — an array of `{iterationId, stage (screen/promote), robustScore, eligible, gatingReason}` accumulated during the run from the existing `score()`/`is_eligible()` call sites and persisted in the `autoRun` block (no new scorer, no new endpoint, no parallel store; survives reload). The Right-panel leaderboard component reads `robustScore`/`eligible`/`gatingReason` **verbatim**, joins each entry to its `iterationHistory` node by `iterationId` for display metrics (return/sharpe/trades/drawdown/`wfe` — no duplicated/recomputed metric), and marks "best" solely by `entry.iterationId === autoRun.bestIterationId` (the one best definition). A higher-raw-return but WFE-failing / over-leveraged candidate is therefore visibly NOT best — its `gatingReason` cites the gate |
| **(Layer-2, iter-3; staged iter-4)** Open-universe search — candidate configs explored (distinct symbol/timeframe drawn from a **bounded seed universe**) | Open-universe controller path in `apps/backend/backend/auto_session.py` (orchestration only — it computes **no** new displayed metric; reuses the existing `BacktestPipeline` + `RobustScorer`; model tier from `shared/model_catalog.py:cheapest_model()`) | `GET /api/sessions/{id}` — writes only the SAME canonical records: candidate configs as iteration `params`, the cross-config winner as `autoRun.bestIterationId` (via the existing `RobustScorer`), counters on `autoRun.budget`, and SCREEN/PROMOTE stage **activity records** | Bounded seed universe only (anti-goal: no exchange-wide fan-out). **(iter-4, J-14) Two-stage cost-tiering:** a cheap **SCREEN** pass evaluates the budget-bounded seed configs on the cheapest catalog model (`cheapest_model()`) with **no walk-forward**; the top-`k` survivors (`k = DEFAULT_PROMOTE_K`, and `k < number screened`) are **PROMOTE**d to full evaluation — walk-forward **and** the stronger request `model` — and the cross-config best is `RobustScorer.select_best()` over the **promoted (WFE-gated) candidates only** (screened-only nodes are never marked best, preserving the WFE-gated-best anti-goal). SCREEN/PROMOTE stages surface as **activity records** in the Left Activity Log (no new value, no new endpoint); any shown score is the one `RobustScorer.score()`. Every unit (each screen + each promote) is bounded by the hard `max_configs` + token/USD + wall-clock budget, checked before the unit. **No parallel store, no new endpoint, no new score.** **(iter-5, J-15) Global-history warm start (opt-out-able):** when the request sets `history_scope: "global"`, the controller mines prior sessions **read-only** (`session_store.derive_session_tabs` / `read_session_meta` / lightweight `read_iteration_meta` — never the eager full `result`/`rating` payload, never mutating prior artifacts) into a per-`(symbol,timeframe)` family leaderboard scored by the **one** `RobustScorer`, runs a **prompt-cached** LLM-planner (`cache_control: ephemeral`, called at most once per run — not re-sent each round; tokens threaded into the one `BudgetTracker`), and reorders the **bounded** seed universe so the historically-strongest in-seed family is screened first and is the first PROMOTEd config. It emits ONE warm-start planner-decision **activity record** (existing `auto-run` type, citing the prior session's performance, no secrets) — no new value, no new endpoint, no parallel store. `history_scope: "this-run"` (the default/opt-out) skips mining + planner + citation and keeps the deterministic seed order. **(iter-7, J-16) Bounded promote breadth:** an optional `promote_k` request field (validated 1–3, **default 1** so J-12/J-13/J-14 stay byte-identical) sets how many top-`k` screened survivors are PROMOTEd to walk-forward + stronger model (`k = min(promote_k, n_screened)`, still budget-gated per promote, still within the bounded seed — never a fan-out). `k ≥ 2` lets the leaderboard show multiple walk-forward-validated candidates so the WFE gate is demonstrably choosing the robust candidate over a higher-raw-return overfit one. The overfit-gated leaderboard *visualization* itself is the `autoRun.leaderboard` row above |
...
endpoint (additive Notes edit only, no nav change). iter-5 lands global-history warm start (J-15): an opt-out-able
(`history_scope: "global"` vs default `"this-run"`) read-only mine of prior sessions + a prompt-cached LLM-planner
that reprioritizes the bounded seed universe so a historically-strong family is screened/promoted first, emitting one
planner-decision activity record (existing `auto-run` type, reusing the one RobustScorer + one BudgetTracker; additive
Notes edit only, no new value/endpoint/nav change). iter-7 lands the overfit-gated leaderboard UI (J-16 — the FINAL journey):
the SAME RobustScorer's per-candidate output is surfaced as `autoRun.leaderboard` (served on the existing `GET /api/sessions/{id}`,
persisted in the autoRun block, read verbatim by a new Right-panel leaderboard component that joins iterationHistory for metrics and
marks best by the one `bestIterationId`), plus a bounded optional `promote_k` (1–3, default 1) so multiple walk-forward-validated
candidates can be ranked and the WFE gate is visibly choosing the robust candidate over a higher-raw-return overfit one — additive
Data-Contract extension of the existing best-marker row, no new endpoint/parallel store/nav change. After J-16: all 16 journeys pass. -->
