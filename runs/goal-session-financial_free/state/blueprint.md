...
| **(Layer-2, iter-3)** Open-universe search — candidate configs explored (distinct symbol/timeframe drawn from a **bounded seed universe**) | Open-universe controller path in `apps/backend/backend/auto_session.py` (orchestration only — it computes **no** new displayed metric; reuses the existing `BacktestPipeline` + `RobustScorer`) | `GET /api/sessions/{id}` — writes only the SAME canonical records: candidate configs as iteration `params`, the cross-config winner as `autoRun.bestIterationId` (via the existing `RobustScorer`), counters on `autoRun.budget` | Bounded seed universe only (anti-goal: no exchange-wide fan-out). Each explored config is evaluated **uniformly** through the existing pipeline (incl. walk-forward — no cheap-`SCREEN` stage yet, so the SCREEN-skips-WF anti-goal is not triggered), bounded by the hard `max_configs` + token/USD + wall-clock budget. **No parallel store, no new endpoint, no new score.** Staged `SCREEN`→`PROMOTE` cost-tiering is **J-14**; the overfit-gated leaderboard *visualization* is **J-16**; the cached LLM-planner warm-start over global history is **J-15** |
...
parallel store, no schema fork). Still forward-looking: staged SCREEN→PROMOTE (J-14), global-history warm start
(J-15), and the overfit-gated leaderboard UI (J-16). -->
