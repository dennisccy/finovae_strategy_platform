...
def _macd_signal(series, fast, slow, signal):
    macd = _macd(series, fast, slow)
    return macd.ewm(span=signal, adjust=False).mean()

def _macd_hist(series, fast, slow, signal):
    macd = _macd(series, fast, slow)
    macd_sig = _macd_signal(series, fast, slow, signal)
    return macd - macd_sig

def _bollinger_upper(series, period, std_dev):
    middle = series.rolling(window=period).mean()
    std = series.rolling(window=period).std()
    return middle + (std * std_dev)

def _bollinger_middle(series, period):
    return series.rolling(window=period).mean()

def _bollinger_lower(series, period, std_dev):
    middle = series.rolling(window=period).mean()
    std = series.rolling(window=period).std()
    return middle - (std * std_dev)
...
def _stoch_k(df, period):
    lowest_low = df["low"].rolling(window=period).min()
    highest_high = df["high"].rolling(window=period).max()
    return 100 * (df["close"] - lowest_low) / (highest_high - lowest_low)

def _stoch_d(df, period, smooth):
    stoch_k = _stoch_k(df, period)
...
'''
    ...
    def _generate_indicator_code(self, indicator: IndicatorConfig) -> str:
        ...
        indicator_code_map = {
            "sma": lambda p: f'indicators["{output_name}"] = _sma(df["close"], {p.get("period", 20)})',
            "ema": lambda p: f'indicators["{output_name}"] = _ema(df["close"], {p.get("period", 20)})',
            "rsi": lambda p: f'indicators["{output_name}"] = _rsi(df["close"], {p.get("period", 14)})',
            "macd": lambda p: f'indicators["{output_name}"] = _macd(df["close"], {p.get("fast_period", 12)}, {p.get("slow_period", 26)})',
            "macd_signal": lambda p: f'indicators["{output_name}"] = _macd_signal(df["close"], {p.get("fast_period", 12)}, {p.get("slow_period", 26)}, {p.get("signal_period", 9)})',
            "macd_hist": lambda p: f'indicators["{output_name}"] = _macd_hist(df["close"], {p.get("fast_period", 12)}, {p.get("slow_period", 26)}, {p.get("signal_period", 9)})',
            "bollinger_upper": lambda p: f'indicators["{output_name}"] = _bollinger_upper(df["close"], {p.get("period", 20)}, {p.get("std_dev", 2.0)})',
            "bollinger_middle": lambda p: f'indicators["{output_name}"] = _bollinger_middle(df["close"], {p.get("period", 20)})',
            "bollinger_lower": lambda p: f'indicators["{output_name}"] = _bollinger_lower(df["close"], {p.get("period", 20)}, {p.get("std_dev", 2.0)})',
            "atr": lambda p: f'indicators["{output_name}"] = _atr(df, {p.get("period", 14)})',
            "stoch_k": lambda p: f'indicators["{output_name}"] = _stoch_k(df, {p.get("period", 14)})',
            "stoch_d": lambda p: f'indicators["{output_name}"] = _stoch_d(df, {p.get("period", 14)}, {p.get("smooth", 3)})',
            "cci": lambda p: f'indicators["{output_name}"] = _cci(df, {p.get("period", 20)})',
            "williams_r": lambda p: f'indicators["{output_name}"] = _williams_r(df, {p.get("period", 14)})',
            "roc": lambda p: f'indicators["{output_name}"] = _roc(df["close"], {p.get("period", 12)})',
            "momentum": lambda p: f'indicators["{output_name}"] = _momentum(df["close"], {p.get("period", 10)})',
            "mfi": lambda p: f'indicators["{output_name}"] = _mfi(df, {p.get("period", 14)})',
        }
